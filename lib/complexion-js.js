...
(function (n, r, f) {
    ...
    function matchKeyword(complexion, state) {
        /*jslint unparam:true*/
        // FIXME:  Consider swapping this function in and out for speed
        return function () {
            var content;
    ...
    function matchMultiLineComment(complexion) {
        return complexion.matchString('/*', function (str, offset) {
            ...
            while (c) {
                // FIXME: consider reworking the double-checking of
                // c for speed
                while (c && c !== '*') {
                    len += 1;
                    c = str.charAt(offset + len);
    ...
    function matchPunctuator(complexion, state) {
        ...
        return function (str, offset, tokenList) {
            ...
            match = hash[str.substr(offset, 4)] || hash[str.substr(offset, 3)] || hash[str.substr(offset, 2)] || hash[str.charAt(offset)] || null;
    ...
    function matchRegularExpressionLiteral() {
        ...
        return function (str, offset, tokenList) {
            ...
            if (str.charAt(offset) !== '/' || !isAllowed(tokenList)) {
                return null;
            }
    ...
    function matchShebang(complexion) {
        // FIXME:  Possibly replace this function after beginning of file
        // for speed
    ...
    return function (complexion, config) {
        ...
        add('NUMERIC_LITERAL', matchNumericLiteral);
        ...
        add('PUNCTUATOR', matchPunctuator);
